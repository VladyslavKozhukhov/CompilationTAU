...
public class SYMBOL_TABLE
{
	...
	public final String SCOPE_BOUNDARY = "SCOPE-BOUNDARY";
	/**********************************************/
	...
	/***********************************************/
	public TYPE find(String name) //search only scope
	{
	...
	public TYPE findWithType(String name,TYPE t) //search only scope
	{
		SYMBOL_TABLE_ENTRY e;

		for (e = table[hash(name)]; e != null; e = e.next)
		{
			if (name.equals(e.name) && e.type.getTypeName().equals(t.getTypeName()))
			{
				return e.type;
			}
		}
	...
	public TYPE findInScopeOrParentClasses(String name){ //search scope and parent classes
		TYPE t;
		t = find(name);

		if(t==null){
			t = TYPE_CLASS.findInParentsDataMembersOfCurrentClass(name);
		}
		return t;
	}
	...
	public TYPE findUntilLevel(String name,int lastLevelToSearch){
		SYMBOL_TABLE_ENTRY e = top;
		int level = GlobalVariables.scopeLevel;
		while (e!=null && level!=lastLevelToSearch-1)
		{
			if(e.name.equals(SCOPE_BOUNDARY)){
				level--;
			}else{
				if(e.name.equals(name)){
					return e.type;
    ...
    public TYPE findFromLevel(String name, int topLevel){
        SYMBOL_TABLE_ENTRY e = top;
        int level = GlobalVariables.scopeLevel;
